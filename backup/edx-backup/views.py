...
#	return HttpResponse(u'<p>request error</p>')
	...
	#create ibm account
	    ...
	    result = CreateIBMAccount(email,password)
            ...
            log.info(email+". create ibm account error")
...
def ibm_password_reset_request(request):
    ...
    mail_user="edx_tsinghua_os"
    mail_pass="tsinghuaos8800"
    mail_postfix="163.com"

    content="<p>hello,the link below will be expired 24 hour later. \n " 
    content += "</p>" + "\n<a href='http://cherry.cs.tsinghua.edu.cn/openedx/ibm/password_reset?usrid=" + usrid + "&vcode=" + vcode +"'>click here to reset your shibboleth password</a>"
...
def AutoLoginGit(username,password):
    ...
    browser.get("http://192.168.1.136/users/sign_in")
    ...
    browser.get("http://192.168.1.136/teacher/ucore_lab/fork/new")
    browser.find_element_by_css_selector("a[title=\"Fork here\"]").click()
...
def CreateLdapAccount(username,password,email):
    ldap_url = "ldap://192.168.1.138:389"
    principal_name = "cn=admin,dc=cscw"
    ldap_password  = "6r4UJxJr1ln"
    base_dn = "ou=Users,dc=cscw"
...
def CreateIBMAccount(email,password):
    headers = {"Content-Type":"application/json","Authorization":"Token 3e49c0482d9675abb1d7c53daca2dc1333a25f08"}
    ret = requests.post("https://dashboard.ptopenlab.com:443/restful/supernova/user/register",headers=headers,data=json.dumps({"user_name":email,"password":password}))
